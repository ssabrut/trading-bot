...
walk-forward:
	set -a && . .env && set +a && poetry run python env/walk_forward.py --fold $(FOLD) $(if $(TIMESTEPS),--timesteps $(TIMESTEPS),) $(if $(ENT_COEF),--ent-coef $(ENT_COEF),) $(if $(LR),--learning-rate $(LR),) $(if $(SEEDS),--seeds $(SEEDS),)
...
walk-forward-all:
	...
	for i in $$(seq 1 $$N_FOLDS); do \
		...
		poetry run python env/walk_forward.py --fold $$i $(if $(TIMESTEPS),--timesteps $(TIMESTEPS),) $(if $(ENT_COEF),--ent-coef $(ENT_COEF),) $(if $(LR),--learning-rate $(LR),) $(if $(SEEDS),--seeds $(SEEDS),) || exit 1; \
	done && \
